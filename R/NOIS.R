...
BIC_NOIS <- function(NOIS_fit, bias_correct = T) {
    ...
    BIC_val <- (npts - m) * log(sum((NOIS_fit$y_adj - fit)^2)/(npts - m)) + (NOIS_fit$pool_q + 1) * (log(npts - m) + 1)
...
#' \item{\code{local_y_adj}}{Unpooled adjusted 'y' values.}
...
#' @export
NOIS_fit <- function(data, x = "x", y = "y", CV_method = "MCV", first_h = NULL, pool_h = NULL, local_q = 0.1, pool_q = 0.1, tol = 1e-07, maxit = 200,
    ...) {
    ...
    for (jj in 1:nn) {
        ...
        for (ii in 1:maxit) {
            ...
            local_fit[jj] <- nwestimator(xx[jj], xx, yy_adj[, jj], first_h)
            rr <- kern_nzsqrt * ((yy - local_fit[jj])[nz_ind])
